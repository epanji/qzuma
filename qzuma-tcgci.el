    ...
    (cond
     ...
     ((string-equal name "read")
      (concat
       ...
       (qz-line (+ ntab 1) 0 "$this->db->where")
       (qz-line 0 neol (format "('%s', $str_or_id);" (car fields)))
     ...
     ((string-equal name "update")
      (concat
       ...
       (qz-line 0 neol (format "('%s', $id);" (car fields)))
		...
		(if (qz-table-p fields)
			(progn
              ...
              (insert
               ...
               (qz-line 0 0 "<a href=\"<?php echo base_url();?>")
               (qz-line 0 0 (format "%s/add/" controller))
               (qz-line 0 0 "\" class=\"btn btn-default\" ><span class=")
